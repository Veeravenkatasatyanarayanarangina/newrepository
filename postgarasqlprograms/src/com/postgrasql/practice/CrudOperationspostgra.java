...
public class CrudOperationspostgra 
{
	public static void main(String[] args) throws ClassNotFoundException, SQLException 
	{
		...
		while (option>4);
		{
			...
			case 2:
			{
				...
				if (updatekey==1)
				{
					ps= connection.prepareStatement("update passenger set pto = ? where pmobno = ?");
					...
					if(record>0)
					{
						...
						ps = connection.prepareStatement("select * from passenger where Pmobno=?");
						ps.setString(1, phno);
				...
				{
					ps =connection.prepareStatement("update passenger set pto = ? where pname = ? and pmobno = ?");
			...
			case 3 :
			{
				...
				if (token==1)
				{
					ps = connection.prepareStatement("delete from passenger where pticketid=?");
				...
				{
					ps=connection.prepareStatement("delete from passenger where pto=?");
			...
			case 4:
			{
				...
				{
					ps = connection.prepareStatement("select * from passenger where pname=? and pmobno=?");
